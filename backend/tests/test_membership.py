def test_get_membership(client, auth_token):
    ...
    response = client.get("/api/membership/John", headers={"Authorization": f"Bearer {auth_token}"})
...
def test_cancel_membership(client, auth_token):
    ...
    response = client.post(
        "/api/membership/John/cancel", headers={"Authorization": f"Bearer {auth_token}"}
    )
    assert response.status_code == 200
