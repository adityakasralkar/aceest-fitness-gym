def test_add_workout(client, auth_token):
    ...
    response = client.post("/api/workouts/John", json={
...
def test_get_workouts(client, auth_token):
    ...
    client.post("/api/workouts/John", json={
        "workout_type": "Cardio",
        "duration_min": 45
    }, headers={"Authorization": f"Bearer {auth_token}"})

    response = client.get("/api/workouts/John", headers={
...
def test_workout_client_not_found(client, auth_token):
    response = client.post("/api/workouts/Nobody", json={
        "workout_type": "Cardio",
        "duration_min": 45
    }, headers={"Authorization": f"Bearer {auth_token}"})
